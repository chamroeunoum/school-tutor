              ...
              <div class="crud-form-panel w-full flex flex-wrap ">
                ...
                >
                  <n-form-item label="ឈ្មោះ" path="name" class="w-4/5 mr-8" >
                    <n-input v-model:value="record.name" clearable placeholder="ឈ្មោះ" />
                  </n-form-item>
                  <n-form-item label="ឈ្មោះទីតាំង" path="origin" class="w-4/5 mr-8" >
                    <n-input v-model:value="record.location_name" clearable placeholder="ឈ្មោះទីតាំង" />
                  </n-form-item>
                  <n-form-item label="អសយដ្ឋាន" class="w-4/5 mr-8" >
                    <n-input v-model:value="record.address" clearable placeholder="អសយដ្ឋាន" />
                  </n-form-item>
                  <n-form-item label="លេខទូរសព័្ធ" class="w-4/5 mr-8" >
                    <n-input v-model:value="record.phone" clearable placeholder="លេខទូរសព័្ធ" />
                  </n-form-item>
...
export default {
  ...
  setup(props){
    ...
    function uploadFiles(){
      ...
      store.dispatch('store/upload', form ).then( res => {
        notify.success({
          title: 'រក្សារទុកព័ត៌មាន' ,
    ...
    function create(){
      ...
      if( record.location_name == "" ){
        notify.warning({
          'title' : 'ពិនិត្យព័ត៌មាន' ,
          'description' : 'សូមបំពេញ ឈ្មោះទីតាំង' ,
          duration : 3000
        })
        return false
      }
      ...
      store.dispatch( props.model.name+'/create',{
        ...
        switch( res.status ){
          case 200 : 
            ...
            record.name = res.data.record.name 
            record.location_name = res.data.record.location_name
            record.address = res.data.record.address
            record.phone = res.data.record.phone
            uploadFiles()
            // clearRecord()
          break;
        }
