  ...
  <!-- Top action panel of crud -->
    <div class="flex title-bar border-b border-gray-200">
      ...
      <div class="flex w-64 h-10 py-1 title " >
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21V8l9-4l9 4v13"></path><path d="M13 13h4v8H7v-6h6"></path><path d="M13 21v-9a1 1 0 0 0-1-1h-2a1 1 0 0 0-1 1v3"></path></g></svg>
    ...
    <div class="vcb-table-panel relative">
      <table class="vcb-table" >
        ...
        <tr v-for="(record, index) in table.records.matched" :key='index' class="vcb-table-row" >
          <td class="vcb-table-cell font-bold" >{{ index + 1 }}
            <!-- <qrcode-vue :value="record.id+''" :size="40" level="H" /> -->
          </td>
          <td class="vcb-table-cell" >{{ record.product.description }}</td>
          <td class="vcb-table-cell" >{{ record.product.upc }}</td>
          <td class="vcb-table-cell" >{{ record.product.origin }}</td>
          <td class="vcb-table-cell" v-html="stockInformation(record)" ></td>
          <td class="vcb-table-cell" >{{ record.attributeVariant.variants.map( v => v.name ).join(', ') }}</td>
          <td class="vcb-table-cell" >{{ record.store != undefined ? record.store.name : 'ឃ្លាំង' }}</td>
          <td class="vcb-table-actions-panel text-right" >
            <n-icon size="22" class="cursor-pointer text-blue-500 mx-1" title="ដកស្ដុក" @click="showStockTransferModal(record)" >
              <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 20 20"><g fill="none"><path d="M11.3 2.48a3.5 3.5 0 0 0-2.6 0L2.943 4.785A1.5 1.5 0 0 0 2 6.176v7.646a1.5 1.5 0 0 0 .943 1.393L8.7 17.518a3.5 3.5 0 0 0 1.376.25a5.495 5.495 0 0 1-.614-1.058a2.49 2.49 0 0 1-.39-.12l-5.758-2.303A.5.5 0 0 1 3 13.822V6.238l6.5 2.6v3.367a5.51 5.51 0 0 1 1-1.48V8.837l6.5-2.6v3.361c.358.183.693.404 1 .658v-4.08a1.5 1.5 0 0 0-.943-1.393L11.3 2.48zm-2.228.93a2.5 2.5 0 0 1 1.857 0l5.225 2.09l-2.279.91l-6.154-2.46l1.35-.54zM6.375 4.487l6.154 2.461L10 7.961L3.846 5.499l2.529-1.011zM14.5 18.999a4.5 4.5 0 1 0 0-9a4.5 4.5 0 0 0 0 9zm.351-6.855l.003.002l2 2a.5.5 0 1 1-.708.707L15 13.707V16.5a.5.5 0 0 1-1 0v-2.793l-1.146 1.146a.5.5 0 1 1-.708-.707l2-2a.5.5 0 0 1 .351-.146h.006a.498.498 0 0 1 .348.144z" fill="currentColor"></path></g></svg>
            </n-icon>
            ...
            <n-icon v-if="record.store_id<=0" size="22" class="cursor-pointer text-green-500 mx-1" title="បញ្ចូលស្តុក" @click="showStockInModal(record)" >
    ...
    <!-- Form Stock Transfer -->
    <stock-transfer-form v-bind:model="model" v-bind:show="stockTransferModal.show" :onClose="closeStockTransferModal" :record="selectedRecord" />
    <!-- Form Unit convention -->
    <unit-convention-form v-bind:model="model" v-bind:show="unitConventionModal.show" :onClose="closeUnitConventionModal" :stock="selectedRecord" />
  </div>
</template>
<script>
...
import StockTransferForm from './transfer.vue'
import StockTransactionForm from './transaction.vue'
import UnitConventionForm from './unitconvention.vue'
export default {
  name: "User" ,
  components: {
    QrcodeVue ,
  ...
  setup(){
    ...
    function showEditModal(record){
      ...
      editRecord.vendor_sku = record.vendor_sku
      editRecord.upc = record.upc
      editRecord.location = record.location
    ...
    /**
     * Stock Transfer modal handling
     */
     var stockTransferModal = reactive({show:false})
    function showStockTransferModal(record){
      selectedRecord.id = record.id
      selectedRecord.store_id = record.store_id
      selectedRecord.product_id = record.product_id
      selectedRecord.unit_id = record.unit_id
      selectedRecord.attribute_variant_id = record.attribute_variant_id
      selectedRecord.quantity = record.quantity
      selectedRecord.product = record.product
      stockTransferModal.show = true
    }

    function closeStockTransferModal(){
      stockTransferModal.show = false
      selectedRecord.id = 0
      selectedRecord.store_id = 0
      selectedRecord.product_id = 0
      selectedRecord.unit_id = 0
      selectedRecord.attribute_variant_id = 0
      selectedRecord.quantity = 0
    ...
    /**
     * Unit Convention Modal
     */
     var unitConventionModal = reactive({show:false})
    function showUnitConventionModal(record){
      selectedRecord.id = record.id
      selectedRecord.product_id = record.product_id
      selectedRecord.unit_id = record.unit_id
      selectedRecord.attribute_variant_id = record.attribute_variant_id
      selectedRecord.quantity = record.quantity
      selectedRecord.product = record.product
      selectedRecord.stockUnits = record.stockUnits
      unitConventionModal.show = true
    }

    function closeUnitConventionModal(){
      unitConventionModal.show = false
      selectedRecord.id = 0
      selectedRecord.product_id = 0
      selectedRecord.unit_id = 0
      selectedRecord.attribute_variant_id = 0
    ...
    function stockInformation(stock){
      return stock.stockUnits !== null ? ( stock.stockUnits.length ? stock.stockUnits.map( 
        stockUnit => { return '+ ' + stockUnit.quantity + " " + stockUnit.unit.name + " - " + stockUnit.sku + " - <b>" + parseFloat( stockUnit.unit_price > 0 ? stockUnit.unit_price : 0 ).toFixed(2) + " $</b>" } 
        ).join('<br/>') : "មិនទាន់មានស្តុក" ) : "មិនទាន់មានស្តុក"
      //  stock.stockUnit.quantity + " - " + stock.stockUnit.unit.name : "មិនទាន់មានស្តុក" ;
    }
    ...
    return {
       ...
       */
       unitConventionModal , 
       showUnitConventionModal ,
       closeUnitConventionModal ,
      /**
       * Functions
       */
      deleteRecord ,
